...
redis_pool: ConnectionPool = aioredis.ConnectionPool(
    ...
    max_connections=5,
    socket_connect_timeout=15,  # Wait up to 15 seconds for connection
)
...
async def get_cache_ttl(
    key: str, redis_client: Redis, ttl: int = TTL_SECONDS
) -> Optional[str]:
    """
...
async def set_cache_ttl(key: str, value: str, redis_client: Redis, ttl: int = TTL_SECONDS):
    ...
    Args:
        ...
        ttl int: Time-to-live in seconds for the key's expiration.
            Defaults to TTL_SECONDS.
    """
    await redis_client.set(key, value, ex=ttl or TTL_SECONDS)
